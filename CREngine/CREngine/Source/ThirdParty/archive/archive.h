...
namespace tser {
    ...
    static constexpr auto g_decodingTable = []() { std::array<unsigned char, 256> decTable{}; for (unsigned char i = 0; i < 64u; ++i) decTable[static_cast<unsigned char>(g_encodingTable[i])] = i; return decTable; }();
